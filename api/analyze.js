export default async function handler(req, res) {
  ...
  // Health check
  if (req.method === 'GET') {
    res.status(200).json({ status: 'ok', message: 'VOCA Gemini API is running.' });
    return;
  }
  ...
  if (!GEMINI_API_KEY) {
    res.status(400).json({ error: "No API key configured on server." });
    return;
  }

  try {
    // Vercel auto-parses req.body if it's application/json
    const body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
    const { transcript, topicTitle, difficulty, targetSpeakingTime, actualSpeakingTime } = body || {};
...
Ensure the output is ONLY valid JSON, with no markdown formatting. Do not include nextChallenge.`;
    ...
    if (!response.ok) {
      ...
      console.error("Error from Gemini:", errData);
      res.status(response.status).json({ error: `Gemini API error: ${response.statusText} - ${errData}` });
      return;
    }
